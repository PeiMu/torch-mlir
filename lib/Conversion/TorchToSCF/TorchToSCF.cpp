...
public:
  ...
  matchAndRewrite(PrimLoopOp op, OpAdaptor adaptor,
    ...
    if (failed(getTypeConverter()->convertTypes(op.getResultTypes(),
                                                newResultTypes)))
      return rewriter.notifyMatchFailure(op,
                                         "could not convert PrimLoopOp outputs");
    ...
    inlineForCase(op.region(), scfFor.region());
...
namespace {
class ConvertTorchPrimLoopConditionOp : public OpConversionPattern<PrimLoopConditionOp> {
  public:
